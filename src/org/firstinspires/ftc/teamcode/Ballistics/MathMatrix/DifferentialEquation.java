        ...
        for (int i = 0; i < total_t / step; i++)
        {
            class func_internal extends MatrixFunction
            {
                ...
                public Matrix execute(MatrixFunction func, Matrix... args) // args = { y_prev }
                {
                    ...
                    return CauchyFunction(2 * i1 + 1, y_n).multiplyByScalar(step / 2).add(args[0]); // FIXME здесь получается неправильный результат вычисления
                }
            }
            func_internal funcint = new func_internal(i);
            y_prev = y_curr;
            y_curr = CauchyFunction((2 * i + 1) * step / 2, funcint).multiplyByScalar(step).add(y_prev); // FIXME вот где юзается CauchyFunction
        }
